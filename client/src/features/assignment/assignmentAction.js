...
export const createAssignment = createAsyncThunk(
    ...
    async (assignmentData) => {
      try {
        const response = await axios.post('http://localhost:3306/api/assignmentDetails/withID', assignmentData);
        const data = response.data;
...
export const fetchAssignmentCounts = createAsyncThunk(
  ...
  async (EmployeeID_AssignTo) => {
    try {
      const response = await axios.get(`http://localhost:3306/api/assignmentDetails/${EmployeeID_AssignTo}/assignmentCounts`);
